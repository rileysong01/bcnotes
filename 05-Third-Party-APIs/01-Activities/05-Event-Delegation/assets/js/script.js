...
function renderLetters() {
  ...
  // Dynamically create buttons for every letter
  // Create a for-loop to iterate through all the letters in the array.
  for (var i = 0; i < letters.length; i++) {
    // Create button
    var letterBtn = $('<button>');
    // Assign style to the button
    letterBtn.addClass('letter-button btn btn-info');
    // Assign each letter to a custom data-letter attribute
    letterBtn.attr('data-letter', letters[i]);
    // Display the letter
    letterBtn.text(letters[i]);
    // Append the letter element
